...
def node_normal2_unk(pos, si):
    s = f"""<div style="position:absolute;left:{int(pos[0])}px;top:{int(-pos[1])}px;">
            <div class="normal_warp node_unknown">
                <div class="node_r1 mc-tooltips">
                    <div class="node_sp"
                    style="width:{si[0]}px;height:{si[1]}px;left:{int((rune_size[0] - si[0]) / 2)}px;top:{int((rune_size[1] - si[1]) / 2)}px;">
                    <span class="mdi mdi-help"></span>
                    </div>
                    <div style="display:none" data-size="350" data-trigger="mouseenter focus">
                        <div class="a" style="color:#d32f2f;"><b>? ? ?</b></div>
                    </div>
                </div>
                <div class="node_r2_score"><svg>
                        <polygon class="score_level" points="8,3 3,13 13,13"></polygon>
...
for node, node_item in nodeDataMap.items():
    ...
    runeId = node_item["runeId"] if "runeId" in node_item else ""
    nodeType = node_item["nodeType"]
    print(node, nodeType, "/" + str(runeId) + "/", pos)

    if nodeType == "NORMAL":
        if runeId != "": # 参见node_52，总之就是给占位符做一个bypass
            ...
            si = [50, 46]
            domstr += node_normal2_unk(pos, si)
    elif nodeType == "START":
        svgstr += plot_svg([pos])
